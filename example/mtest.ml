...
let main () =
  ...
  let env = Env.create ~maxreaders:1 ~mapsize:10485760 ~flags:Lmdb_types._MDB_FIXEDMAP ~mode:0o664 db in
  ...
  for i = 0 to count - 1 do
    ...
    let data = Input.of_string (Printf.sprintf "%03x %d foo bar" values.(i) values.(i)) in
    (* TODO: the error type should be reified here to allow to match on some return codes. *)
    try put txn dbi ~key ~data ~flags:Lmdb_types._MDB_NOOVERWRITE
    with _ -> Int.incr j
  done;
  ...
  Cursor.run txn dbi ~f:(fun cursor ->
      while try Cursor.next cursor; true with _ -> false do
        let key, data = Cursor.get_current cursor
            ~key:Cursor.Output.Allocate_bytes
            ~data:Cursor.Output.Allocate_bytes
        in
        printf "key: %s, data: %s\n" (Bytes.to_string key) (Bytes.to_string data)
      done
    );
  Txn.abort txn;
